...
interface KycRejectionListProps {
  ...
  onItemClick?: (reason: RejectionReasonItem) => void;
}

export default function KycRejectionList({ rejectionReasons, rejectedReason, onItemClick }: KycRejectionListProps) {
  return (
    <div className="w-full mb-10">
      ...
      <div className="space-y-3">
        {rejectionReasons.length > 0 ? rejectionReasons.map((reason, index) => (
          <div key={index} onClick={() => onItemClick?.(reason)} className="flex items-center gap-4 p-4 bg-red-100 border border-red-300 rounded-[16px] hover:border-red-500 transition-all cursor-pointer group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11.5px] text-gray-600 leading-[16px]" style={{ fontFamily: "'Bai Jamjuree', sans-serif" }}>
                {reason.documentType ? reason.title : ''}{reason.documentType && reason.detail ? ' — ' : ''}{reason.detail}
              </p>
            </div>
            <Icon name="chevron_right" color="#f20d0dff" style={{ fontSize: '18px' }} className="group-hover:text-[#94A3B8] transition-colors" />
          </div>
        )) : (
